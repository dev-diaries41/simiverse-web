...
<section id="settings" className="fixed inset-0 flex items-center justify-center z-[1000] max-w-[100%] lg:max-w-[60%] mx-auto">
  <div className="fixed inset-0 bg-black opacity-80 blur-2xl pointer-events-none"></div>

  <div className="relative flex-1 opacity-90 shadow-md rounded-lg font-sans text-left text-sm ">
    <div className="absolute inset-0 bg-gradient-radial from-cyan-400/30 via-blue-500/20 to-gray-900/40 opacity-70 blur-xl pointer-events-none rounded-lg"></div>
    <div className="relative flex-grow flex-col min-h-[500px] overflow-y-auto w-full rounded-lg border border-blue-900">
      ...
      <div className="flex flex-col md:flex-row w-full">
        <div className="flex flex-row md:flex-col w-full max-w-[1000%] md:max-w-[20%] justify-start p-4 gap-2">
          {simulationSettings.map((settingCategory, index) => (
            <div
              key={index}
              className={`${selectedSettingCategory === settingCategory.name ? 'bg-gray-700' : ''} rounded-md p-2 mb-2`}
            >
              <h3
        ...
        <div className="flex flex-col w-full p-5">
          {(simulationSettings.filter(setting => setting.name === selectedSettingCategory)[0]).items.map((item, idx) => (
            <div key={idx} className="w-full flex justify-between items-center mb-3">
              ...
              {item.type === 'button' && (
                <button
                  className={`flex-1 ml-3 p-2 text-xs ${item.label.toLowerCase().includes('delete') ? 'bg-red-600 hover:bg-red-500 ' : 'bg-gray-700 hover:bg-gray-600 '} rounded-full max-w-[20%]`}
